...
def analyze():
    ...
    summary = generate_ai_summary(df.copy(), date_col, cat_col, res_col, client=client)
...
def chat():
    ...
    # reuse chatbot logic which expects a dataframe or csv
    try:
        df = pd.read_csv(io.StringIO(csv_sample))
    except Exception:
        df = pd.DataFrame()
    # call chatbot_query with dataframe and question
    answer = chatbot_query(df, question, client=client)
